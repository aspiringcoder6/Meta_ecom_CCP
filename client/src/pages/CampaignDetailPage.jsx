...
export default function CampaignDetailPage() {
  ...
  const { user } = useAuth()
  const { campaigns, creators, isLoadingCampaigns, campaignBackendAvailable, addCampaignCreators, createAndAssignCampaignCreator, removeCampaignCreator, updateCampaignCreator, updateCampaignMilestones, updateCampaignStatus, markCampaignClientChangesRead, refreshCampaign, ensureCampaignReviewLink, showToast } = useApp()
  ...
  return (
    <main className="page campaign-detail-page">
      ...
      {activeTab === 'internal-listings' && <CampaignInternalListingsTab campaign={campaign} creators={creators} canEdit={canEdit} onAddCreators={(ids) => addCampaignCreators(campaign.id, ids)} onQuickAdd={(form) => createAndAssignCampaignCreator(campaign.id, form)} onRemoveCreator={(creatorId) => removeCampaignCreator(campaign.id, creatorId)} onUpdateCreator={(creatorId, changes) => updateCampaignCreator(campaign.id, creatorId, changes)} onMarkChangesRead={() => markCampaignClientChangesRead(campaign.id)} onNotify={showToast} />}
