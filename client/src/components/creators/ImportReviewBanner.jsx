  ...
  return (
    <div className="import-review-banner">
      <div className={`import-review-summary ${showPreview ? '' : 'is-collapsed'}`}>
        ...
        <div><strong>{review.completed ? 'Đã hoàn tất import' : 'Xem preview'}: {review.fileName}</strong>{showPreview && <p>{review.completed ? `Đã lưu ${review.savedCreatedCount} Creator mới, cập nhật ${review.savedUpdatedCount} Creator và bỏ qua ${review.skippedOnSave} dòng không hợp lệ.` : review.mode === 'append' ? `Sẽ thêm ${review.createdCount} Creator mới và cập nhật ${review.updatedCount} Creator hiện có.` : `Sẽ thay toàn bộ danh sách bằng ${review.importedCount} Creator từ file.`}</p>}</div>
        <div className="import-review-actions">
          {showPreview && <div className="import-review-stats"><span className="has-created"><strong>{review.completed ? review.savedCreatedCount : review.createdCount}</strong> mới</span><span className="has-updated"><strong>{review.completed ? review.savedUpdatedCount : review.updatedCount}</strong> cập nhật</span><span><strong>{review.unchangedCount}</strong> không đổi</span><span className={review.duplicateCount ? 'has-warning' : ''}><strong>{review.duplicateCount}</strong> trùng trong file</span><span className={review.errorCount ? 'has-error' : ''}><strong>{review.errorCount}</strong> lỗi</span></div>}
      ...
      {review.saveError && <section className="import-save-error" role="alert"><Icon name="warning" size={17} /><div><strong>Import chưa được lưu vào database</strong><p>{review.saveError}</p><small>Dữ liệu preview vẫn được giữ lại. Hãy kiểm tra rồi bấm “Lưu các dòng hợp lệ” để thử lại, hoặc hủy import.</small></div></section>}
