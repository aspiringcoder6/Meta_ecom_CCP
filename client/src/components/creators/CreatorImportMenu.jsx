  ...
  return (
    <div className={`creator-import-menu${disabled ? ' is-disabled' : ''}`} data-tour={tourId} title={disabled ? disabledReason : undefined}>
      ...
      {!disabled && <div className="import-menu-popover">
        ...
        <button onClick={() => appendInput.current?.click()}><span className="import-option-icon is-add"><Icon name="plus" size={17} /></span><span><strong>Thêm và cập nhật</strong><small>Creator trùng TikTok ID sẽ được gộp Category và cập nhật dữ liệu</small></span></button>
