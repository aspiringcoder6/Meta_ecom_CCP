...
export default function CreatorDetailsDrawer({ creator, onClose, onArchive, onEdit }) {
    ...
    <div className="drawer-layer">
      ...
      <aside className="creator-drawer">
        ...
        <section className="profile-section">
          <div className="section-title"><h4>Thông tin Creator</h4><button type="button" onClick={() => onEdit(creator.id)}><Icon name="edit" size={15} /> Chỉnh sửa</button></div>
          <dl className="profile-details"><div><dt>Segment</dt><dd>{creator.segment}</dd></div><div><dt>Category</dt><dd>{creator.category}</dd></div><div><dt>Type</dt><dd>{creator.type}</dd></div><div><dt>Cost</dt><dd>{formatCurrency(creator.cost)}</dd></div><div><dt>Extra/FOC</dt><dd>{formatCurrency(creator.extraCost)}</dd></div><div><dt>Tổng Cast</dt><dd>{formatCurrency(pricing.totalCast)}</dd></div><div><dt>Booking Expense</dt><dd>{formatCurrency(pricing.bookingExpense)}</dd></div></dl>
        </section>
        ...
        <div className="drawer-actions"><button className="secondary-button" type="button" onClick={() => onEdit(creator.id)}><Icon name="edit" />Chỉnh sửa hồ sơ</button><button className="danger-text-button" type="button" onClick={() => onArchive(creator.id)}><Icon name="archive" />{creator.status === 'Archived' ? 'Khôi phục' : 'Lưu trữ'}</button></div>
      </aside>
    </div>
  )
}
