...
export default function CreatorWorkspace({ creators, allCreators = creators, filters, filterOptions, numericFilters, sortCriteria, categoryDisplayLevel = 1, isFullscreen = false, canManage = false, canUndo = false, canRedo = false, recentlyAddedCreatorId, importReview, onFilterChange, onAddNumericFilter, onRemoveNumericFilter, onReset, onSort, onCategoryDisplayLevelChange, onSelect, onArchive, onUpdateCreator, onDeleteCreator, onQuickAdd, onUndo, onRedo, onBeginEdit, onCommitEdit, onCancelEdit, onAcceptImport, onCancelImport, onImport, onExport, onAddCreator, onEnterFullscreen, onExitFullscreen }) {
  ...
  useEffect(() => {
    if (isFullscreen && importReview && !importReview.completed) setEditMode(true)
  ...
  return (
    <section className={`panel creators-panel ${isFullscreen ? 'creator-fullscreen-view' : ''} ${editMode ? 'fullscreen-edit-mode' : ''} ${isSaving ? 'is-saving' : ''}`} aria-busy={isSaving}>
      {isFullscreen && (
        <>
          <header className="fullscreen-header" data-tour="fullscreen-header" style={{ height: `${headerHeight}px` }}>
            <div><span className="fullscreen-kicker">Creator Management</span><h2>Danh sách Creator</h2><p>{isSaving ? 'Đang lưu và xác nhận dữ liệu với database...' : importReview?.completed ? `Đã hoàn tất import · ${importReview.fileName}` : importReview ? `Xem trước import · ${importReview.fileName}` : editMode ? 'Chế độ chỉnh sửa spreadsheet' : `${creators.length} kết quả phù hợp`}</p></div>
            <div className="fullscreen-actions">
              ...
              {canManage && (importReview ? importReview.completed
                ? <button className="mode-toggle-button is-editing" onClick={cancelImport}><Icon name="check" size={16} />Đóng kết quả</button>
                : <><button className="cancel-edit-button" disabled={isSaving} onClick={cancelImport}><Icon name="close" size={15} />Hủy import</button><button className="mode-toggle-button is-editing" disabled={isSaving} onClick={acceptImport}><Icon name={isSaving ? 'clock' : 'check'} size={16} />{isSaving ? 'Đang lưu...' : 'Lưu các dòng hợp lệ'}</button></>
                : <>{editMode ? <button className="quick-add-button" disabled={isSaving} onClick={onQuickAdd}><Icon name="plus" />Thêm nhanh</button> : <button className="primary-button" onClick={onAddCreator}><Icon name="plus" />Thêm Creator</button>}{editMode && <button className="cancel-edit-button" disabled={isSaving} data-tour="cancel-edit" onClick={cancelEditMode}><Icon name="close" size={15} />Hủy thay đổi</button>}<button className={`mode-toggle-button ${editMode ? 'is-editing' : ''}`} disabled={isSaving} data-tour="edit-toggle" onClick={editMode ? finishEditMode : enterEditMode}><Icon name={isSaving ? 'clock' : editMode ? 'check' : 'edit'} size={16} />{isSaving ? 'Đang lưu...' : editMode ? 'Hoàn tất' : 'Chỉnh sửa'}</button></>)}
      ...
      <div className="table-meta"><span><strong>{creators.length}</strong> Creator</span><span>{importReview?.completed ? 'Các dòng hợp lệ đã được lưu · những dòng bị bỏ qua và nguyên nhân được liệt kê phía trên' : importReview ? 'Dòng xanh lá: Creator mới · dòng xanh dương: Creator được cập nhật · lỗi được liệt kê màu đỏ phía trên' : editMode ? 'Bấm vào ô để sửa · Enter để lưu · Esc để hủy nội dung đang nhập' : isFullscreen ? 'Bấm header để sort nhiều tiêu chí · Kéo mép cột để chỉnh độ rộng' : 'Bấm header để sort nhiều tiêu chí · Cuộn ngang để xem toàn bộ thông tin'}</span></div>
