...
export function useCampaignTour({ mode, canManage = false, activeTab = 'overview', openTab }) {
  ...
  const startTour = useCallback(async () => {
    ...
    const listSteps = [
      ...
      step('[data-tour="campaigns-list"]', 'Danh sách Campaign', 'Bấm một dòng để mở workspace riêng của Campaign, nơi quản lý listing, timeline, deliverables và Final Creators.', 'top'),
    ...
    const createSteps = [
      ...
      step('[data-tour="campaign-create-general"]', 'Thông tin chung', 'Nhập tên Campaign, Client/Brand, Owner và mô tả mục tiêu hoặc thông điệp chính.'),
    ...
    const detailSteps = [
      step('[data-tour="campaign-detail-heading"]', 'Workspace của Campaign', 'Đây là khu vực làm việc riêng cho Campaign hiện tại. Guide sẽ đi lần lượt qua workflow từ Internal Listings đến Final Creators.', 'bottom', 'start'),
      ...
      step('[data-tour="campaign-tabs"]', 'Các phần của Campaign', 'Dùng các tab để quản lý danh sách nội bộ, dữ liệu gửi Brand, timeline, quá trình thực thi và Final Creators.'),
      step('[data-tour="campaign-overview-metrics"]', 'Overview theo workflow mới', 'Bốn chỉ số lần lượt phản ánh Internal Listings, Brand Approved, Deliverables Done và Final Creators.'),
      ...
      step('[data-tour="campaign-overview-deliverables"]', 'Tiến độ execution', 'Theo dõi tổng deliverable đã Done, đang xử lý hoặc Cancel của các KOC đã được Brand và KOC cùng duyệt.'),
      step('[data-tour="campaign-overview-finance"]', 'Thông tin và ngân sách', 'Expense dự kiến lấy từ Internal Listings; Final Expense chỉ tính các Creator đã hoàn tất toàn bộ deliverable, sau đó cộng agency fee 6%.'),
      ...
      tabStep('timeline', 'Timeline', 'Theo dõi deadline, milestone, người phụ trách và cảnh báo sắp đến hạn.', '[data-tour="campaign-timeline-workspace"]'),
      step('[data-tour="campaign-timeline-workspace"]', 'Timeline trực quan', canManage ? 'Bấm “Chỉnh sửa Timeline” để thêm milestone, ngày dự kiến, owner và trạng thái. Mốc trong 3 ngày sẽ vào hệ thống thông báo.' : 'Timeline hiển thị tiến độ, mốc sắp tới, quá hạn và người phụ trách.'),
      ...
      step('[data-tour="campaign-deliverables-workspace"]', 'Execution Tracking', 'Mỗi Creator có thể có nhiều dòng deliverable; Quantity là tổng số dòng. Cập nhật tiến độ, SDHA, Product, demo, feedback, lịch air và Code Ads tại đây.'),
      ...
      tabStep('final-creators', 'Final Creators', 'Mở danh sách KOC đã hoàn thành đầy đủ điều kiện.', '[data-tour="campaign-final-workspace"]'),
      step('[data-tour="campaign-final-workspace"]', 'Final Creators và thanh toán', 'Theo dõi Cost, Extra, Cast, AGI, Expense, tổng ngân sách có agency fee 6%, cùng Tracking và Note thanh toán nhập tay. Bạn đã hoàn tất Campaign guide.', 'top'),
    ]
