...
export function useCampaignTour({ mode, canManage = false, activeTab = 'overview', openTab }) {
  ...
  const startTour = useCallback(async () => {
    ...
    const detailSteps = [
      ...
      step('[data-tour="campaign-overview-deliverables"]', 'Ưu tiên tiến độ Deliverables', 'Năm trạng thái cần theo dõi gồm Done, Lên kịch bản, Lên Demo, Brand Feedback và Duyệt chờ air. Bấm một ô để mở Deliverables với filter tương ứng.'),
      ...
      step('[data-tour="campaign-deliverables-table"]', 'Bảng Deliverables linh hoạt', 'Link TikTok, ID TikTok và Tiến độ luôn hiển thị. Dùng Xem đầy đủ để mở nhóm thông tin Creator và SDHA; các metric phía trên có thể áp dụng nhanh filter Tiến độ.', 'top', 'start'),
      tabStep('financial-listings', 'Financial Listings', 'Mở bảng tài chính của KOC đã được Brand duyệt và KOC xác nhận.', '[data-tour="campaign-final-workspace"]'),
